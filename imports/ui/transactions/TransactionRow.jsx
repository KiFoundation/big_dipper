...
export const TransactionRow = (props) => {
    let tx = props.tx;
    // console.log(tx);
    return <SentryBoundary><Row className={(tx.code)?"tx-info invalid":"tx-info" + " bg-white my-2 py-3 list-border vertical-middle"}>
        ...
        <Col xs={(!props.blockList)?{size:6}:{size:12}} md={(!props.blockList)?{size:3}:{size:7}} lg={(!props.blockList)?{size:1}:{size:2}} className="text-truncate"><i className="fas fa-hashtag d-lg-none"></i> <Link to={"/transactions/"+tx.txhash}>{tx.txhash}</Link></Col>
